...
object GenCompl {
  ...
  // generate completion file "$SYNTH262_HOME/.completion"
  def update: Unit = dumpFile(content, path)

  // commands
  val commands = Synth262.commands

  // phases
  val phases = Synth262.phases

  // global options
  val options = Synth262.options
  ...
  def content: String = {
    ...
    |fi
    |
    |# completion for synth262
    |_synth262_completions() """.stripMargin
    app.wrap {
    ...
    app :> "complete -o filenames -o bashdefault -F _synth262_completions synth262"
