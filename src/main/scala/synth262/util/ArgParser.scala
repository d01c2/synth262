...
import synth262.{Synth262, Command, CommandConfig}
import synth262.phase.PhaseOption
...
class ArgParser(cmd: Command[_], cmdConfig: CommandConfig)
  ...
  addRule(cmdConfig, "", Synth262.options)
              ...
              case obj if json.isObject =>
                obj.asObject.get.toList.foreach {
                  case (phase, value) if value.isObject => {
                    if (Synth262.phases.map(_.name).contains(phase))
