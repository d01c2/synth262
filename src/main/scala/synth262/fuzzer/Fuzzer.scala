package synth262.fuzzer

import synth262.cfg.*
import synth262.error.*
import synth262.es.*
import synth262.es.util.*
import synth262.fuzzer.mutator.*
...
import synth262.{Synth262, FUZZ_LOG_DIR, LINE_SEP}
...
class Fuzzer(
  ...
  lazy val result: Coverage = {
    ...
    if (log) {
      ...
      dumpFile(Synth262.currentVersion, s"$logDir/version")
  ...
  def handleResult(code: String, result: Try[(Boolean, Boolean)]): Boolean = {
    ...
    val pass = result match
      ...
      case Failure(e: TimeoutException) =>
        ...
        false
      case Failure(e: Synth262Error) =>
        debugging(failMsg("SYNTH262 ERROR"))
        synth262Errors += e -> (synth262Errors.getOrElse(e, Set()) + code)
        false
      case Failure(e) =>
        e.getMessage match
  ...
  // Synth262 errors collected during fuzzing
  private val synth262Errors: MMap[Synth262Error, Set[String]] = MMap()
  ...
  private def logging: Unit =
    ...
    // dump Synth262 errors
    dumpFile(
      name = "found Synth262 errors",
      data = synth262Errors.toVector
        .sortBy(_._1.errMsg)
        .map { (e, codes) =>
      ...
      filename = s"$logDir/synth262-errors",
    )
