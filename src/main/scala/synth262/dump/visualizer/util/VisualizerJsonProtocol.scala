...
import synth262.cfg.*
import synth262.dump.visualizer.*
import synth262.error.Synth262Error
...
class VisualizerJsonProtocol(cfg: CFG) {
  def funcNameToFeature(fName: String): Feature = {
    ...
    func.head match
      ...
      case _ =>
        throw Synth262Error(s"invalid feature $fName")
  }
  ...
  def parseNodeName(name: String): Int = {
    ...
    result match
      ...
      case _ => throw Synth262Error(s"invalid node name $name")
  }
  ...
  def parseCallPath(path: Option[String]): Option[String] = {
    def parse(path: String): String = {
      ...
      result match
        ...
        case _ => throw Synth262Error(s"invalid call path $path")
    }
    path match
      case None       => None
