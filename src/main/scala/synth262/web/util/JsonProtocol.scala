package synth262.web.util

import synth262.cfg.*
import synth262.es.{Ast, Lexical, Syntactic}
import synth262.ir.{Func as IRFunc, FuncKind}
import synth262.util.*
import synth262.util.BasicJsonProtocol
import io.circe.*, io.circe.syntax.*, io.circe.generic.semiauto.*

class JsonProtocol(cfg: synth262.cfg.CFG) extends BasicJsonProtocol {
        ...
        case (name, f) =>
          (
            ...
            (
              f.name,
              f.kind.ordinal,
              f.params.map((p: synth262.ir.Param) =>
                (
                  p.lhs.name,
                  p.optional,
                  p.ty.toString,
